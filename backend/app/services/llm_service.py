...
class MCQQuestion(BaseModel):
    """A multiple-choice question generated by LLM."""
    question_id: str = Field(..., description="UUID identifying this question in Redis")
    question: str = Field(..., description="The question text")
    list_answer: List[str] = Field(..., description="Shuffled list of 3 answer choices")
    correct_answer: str = Field(..., description="Text of the correct answer")
...
class LLMService:
    ...
    def generate_mcq_question(
        ...
        """Generate a multiple-choice question and shuffle the answer choices.

        Returns MCQQuestion with a UUID question_id, the question text,
        a shuffled list_answer, and the correct_answer text.
        """
        if not lesson_content or not lesson_content.strip():
            raise ValueError("lesson_content cannot be empty")
        if not concepts or not any(c.strip() for c in concepts):
        ...
        prompt = LLMPrompts.mcq_question_generation(
            lesson_content=lesson_content,
            concepts=concepts,
            conversation_history=conversation_history,
            course_topic=course_topic,
        ...
        try:
            message = self.client.chat.completions.create(
                ...
                max_tokens=500,
                messages=[
                    {"role": "system", "content": "Bạn tạo câu hỏi trắc nghiệm. Chỉ trả về JSON object theo đúng format, không markdown."},
                    {"role": "user", "content": prompt},
                ],
            )
            ...
            choices = [correct] + data["distractors"]
            random.shuffle(choices)

            question_id = str(uuid4())
            logger.info(f"Generated MCQ question_id={question_id}, concepts={len(concepts)}")
            return MCQQuestion(
            ...
            logger.error(f"API error when generating MCQ: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error generating MCQ: {e}")
            raise
