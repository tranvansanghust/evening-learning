...
class OnboardingService:
    ...
    def load_and_enroll_preset_course(self, user_id: int, slug: str) -> Optional[Lesson]:
        """Load a pre-built course from disk and enroll the user.

        Reuses an existing preset course record in DB if one already exists
        (avoids duplicates across multiple users). Always creates a new
        UserCourse enrollment for this user.
        """
        from app.services.lesson_loader import LessonLoader
        data = LessonLoader().load_course(slug)
        if not data:
            logger.warning(f"Preset course '{slug}' not found on disk")
            return None

        # Reuse existing preset course to avoid duplicates
        course = self.db.query(Course).filter(
            Course.source == "preset",
            Course.source_id == slug,
        ).first()
        ...
        if not course:
            ...
            # Set content_markdown from disk (not generated by LLM)
            db_lessons = (
            ...
            for db_lesson, lesson_data in zip(db_lessons, data["lessons"]):
                db_lesson.content_markdown = lesson_data["content_markdown"]
            self.db.commit()
            logger.info(f"Created preset course '{slug}' (id={course.course_id})")

        self.save_user_course_enrollment(user_id, course.course_id)
        return self.get_first_lesson(course.course_id)
